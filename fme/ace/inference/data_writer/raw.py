...
def _ensure_datetime_array(
    ...
    """Convert numpy.datetime64 arrays to datetime objects for cftime."""
    if isinstance(values, np.ma.MaskedArray):
        values = values.filled(np.datetime64("NaT"))
    array = np.asarray(values)
    if not isinstance(array, np.ndarray):
        return array
    if np.issubdtype(array.dtype, np.datetime64):
        array_us = array.astype("datetime64[us]")
        flat_us = array_us.reshape(-1).astype(np.int64)
        if np.any(flat_us == np.iinfo(np.int64).min):
            raise ValueError("Encountered NaT while converting numpy.datetime64 values")
        epoch = datetime.datetime(1970, 1, 1)
        converted = np.array(
            [epoch + datetime.timedelta(microseconds=int(us)) for us in flat_us],
            dtype=object,
        )
...
class RawDataWriter:
    ...
    def append_batch(
        ...
        batch_time_values = _ensure_datetime_array(batch_time.values)

        if start_timestep == 0:
            init_times: np.ndarray = _ensure_datetime_array(
                batch_time.isel(time=0).values
            )
...
def get_batch_lead_time_microseconds(
    init_time: np.ndarray, batch_time: np.ndarray
    ...
    Args:
        init_time: Initialization time for each sample in the batch (datetime or cftime).
        batch_time: Time coordinates for each sample in the batch (matching type).
