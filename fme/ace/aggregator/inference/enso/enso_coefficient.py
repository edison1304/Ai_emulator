...
def get_sample_index_series(
    ...
    # Harmonize time dtypes: use numpy datetime64 when initial_time is numpy, else use cftime
    initial_is_numpy_datetime = np.issubdtype(initial_time.dtype, np.datetime64)
    target_use_cftime = not initial_is_numpy_datetime
    index_data = index_data.convert_calendar(
        calendar=data_calendar,
        dim="time",
        use_cftime=target_use_cftime,
    )
    sample_index_series: list[xr.DataArray | None] = []
    for initial_time_sample in initial_time:
        # Use a numpy timedelta when initial_time uses numpy datetime64, otherwise
        # fall back to a python timedelta for cftime datetimes.
        if initial_is_numpy_datetime:
            seconds_per_step = int(timestep.total_seconds())
            duration = np.timedelta64(n_forward_timesteps * seconds_per_step, "s")
        else:
        ...
        t0 = index_data.time[0].item()
        t1 = index_data.time[1].item()
        if target_use_cftime:
            index_timestep_seconds = (t1 - t0).total_seconds()
        else:
            # Convert to timedelta64 first, then divide to get seconds
            index_timestep_seconds = float((np.timedelta64(t1 - t0, 'ns') / np.timedelta64(1, "s")))
        if initial_is_numpy_datetime:
            half_index_timestep = np.timedelta64(int(index_timestep_seconds / 2), "s")
        else:
            half_index_timestep = datetime.timedelta(seconds=index_timestep_seconds / 2)
        # Use isel instead of sel to avoid datetime compatibility issues
        try:
            ...
            # Fallback: use time-based indexing with tolerance
            start_time = initial_time_sample - half_index_timestep
            end_time_slice = end_time + half_index_timestep

            # Find closest time indices
            if target_use_cftime:
                times = index_data.time.values
                start_diffs = np.array(
                    [abs((t - start_time).total_seconds()) for t in times]
                )
                end_diffs = np.array(
                    [abs((t - end_time_slice).total_seconds()) for t in times]
                )
                start_idx = int(start_diffs.argmin())
                end_idx = int(end_diffs.argmin())
            else:
                start_diffs = np.abs(index_data.time.values - start_time)
                end_diffs = np.abs(index_data.time.values - end_time_slice)
                start_idx = int(start_diffs.argmin())
                end_idx = int(end_diffs.argmin())
            ...
            sample_index_data_selection = index_data.isel(time=slice(start_idx, end_idx + 1))
        if sample_index_data_selection.sizes["time"] == 0:
            ...
            # Convert time values to proper datetime format for comparison
            time_start = sample_index_data_selection.time[0].item()
            time_end = sample_index_data_selection.time[-1].item()
            
            # Handle different time formats
            if hasattr(time_start, 'to_datetime64'):
                time_start = time_start.to_datetime64()
            if hasattr(time_end, 'to_datetime64'):
                time_end = time_end.to_datetime64()
            
            valid_sample_time = sample_time.where(
                np.logical_and(
                    sample_time >= time_start,
                    sample_time <= time_end,
                ),
            ).dropna()
