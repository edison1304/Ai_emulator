...
class Optimization(OptimizationABC):
    ...
    def step_weights(self):
        ...
        should_step = self._accumulated_batches >= self._gradient_accumulation_steps
        if not self._use_gradient_accumulation and should_step:
        ...
        if should_step:
            ...
            self._accumulated_loss = torch.tensor(0.0, device=get_device())
            self._accumulated_batches = 0
        elif self._use_gradient_accumulation:
            self._accumulated_loss = torch.tensor(0.0, device=get_device())
...
class OptimizationConfig:
    ...
    Parameters:
        ...
        gradient_accumulation_steps: Number of training batches to accumulate before
            applying an optimizer step. Must be a positive integer.
    """
