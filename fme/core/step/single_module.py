...
def step_with_adjustments(
    ...
    Returns:
        The denormalized output data at the next time step.
    """
    # Diagnostics: check NaNs in inputs
    try:
        with torch.no_grad():
            any_input_nan = False
            for _name, _t in input.items():
                if torch.isnan(_t).any():
                    any_input_nan = True
                    logging.warning(
                        f"[NaN-Check] step input contains NaN in '{_name}'"
                    )
            if not any_input_nan:
                logging.debug("[NaN-Check] step input: no NaNs detected")
    except Exception:
        pass

    # Move inputs to the active device before normalization to accelerate ops on GPU
    try:
        from fme.core.device import get_device
        device = get_device()
        input = {k: v.to(device) for k, v in input.items()}
        next_step_input_data = {k: v.to(device) for k, v in next_step_input_data.items()}
    except Exception:
        pass
    ...
    # Diagnostics: check NaNs in normalized network outputs
    try:
        with torch.no_grad():
            any_out_norm_nan = False
            for _name, _t in output_norm.items():
                if torch.isnan(_t).any():
                    any_out_norm_nan = True
                    logging.warning(
                        f"[NaN-Check] output_norm contains NaN in '{_name}'"
                    )
            if not any_out_norm_nan:
                logging.debug("[NaN-Check] output_norm: no NaNs detected")
    except Exception:
        pass
    ...
    # Diagnostics: check NaNs after corrections
    try:
        with torch.no_grad():
            any_output_nan = False
            for _name, _t in output.items():
                if torch.isnan(_t).any():
                    any_output_nan = True
                    logging.warning(
                        f"[NaN-Check] step output (post-corrector/ocean) contains NaN in '{_name}'"
                    )
            if not any_output_nan:
                logging.debug("[NaN-Check] step output: no NaNs detected")
    except Exception:
        pass
    return output
