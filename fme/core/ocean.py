...
class Ocean:
    ...
    def __call__(
        ...
        # Align replacement temperature shape to generated field for safe masking
        try:
            original = gen_data[self.surface_temperature_name]
            repl = next_step_temperature
            # If leading dim mismatches batch, prefer selecting first slice on repl
            if repl.dim() == original.dim() and repl.shape[0] != original.shape[0]:
                if repl.shape[0] == 1:
                    repl = repl.expand(original.shape[0], *repl.shape[1:])
                else:
                    repl = repl.select(0, 0)
            # Adjust rank: squeeze extra singleton dims in repl, then pad with singleton dims
            while repl.dim() > original.dim():
                squeezed = False
                for d in range(repl.dim()):
                    if repl.shape[d] == 1:
                        repl = repl.squeeze(d)
                        squeezed = True
                        break
                if not squeezed:
                    # fallback: drop earliest extra dim
                    repl = repl.select(0, 0)
            while repl.dim() < original.dim():
                repl = repl.unsqueeze(0)
            # Expand singleton dims to match original shape where possible
            expand_shape = []
            for so, sr in zip(original.shape, repl.shape):
                if sr == so:
                    ...
                    expand_shape.append(sr)
            repl = repl.expand(*expand_shape)
            next_step_temperature = repl
        except Exception:
            pass
