...
def _prepend_timesteps(
    ...
    def _align_dims(t: torch.Tensor, target: torch.Tensor, time_dim: int) -> torch.Tensor:
        # Squeeze redundant singleton dims not corresponding to time_dim until dims match
        while t.dim() > target.dim():
            squeezed = False
            for d in range(t.dim()):
                if d != time_dim and t.shape[d] == 1:
                    t = t.squeeze(d)
                    squeezed = True
                    break
            if not squeezed:
                break
        # If still mismatched, try unsqueezing missing dims as singleton except time_dim
        while t.dim() < target.dim():
            # insert singleton before spatial dims to align
            insert_pos = min(max(time_dim, 0), t.dim())
            t = t.unsqueeze(insert_pos if insert_pos < t.dim() else t.dim())
        return t

    return EnsembleTensorDict(
        {
            k: torch.cat([_align_dims(timesteps[k], v, time_dim), v], dim=time_dim)
...
class Stepper(
    ...
    def train_on_batch(
        ...
        # Batch-level NaN scan before any slicing to identify loader issues
        try:
            with torch.no_grad():
                for _name, _t in data.data.items():
                    _nan = torch.isnan(_t)
                    if _nan.any():
                        _nan_count = int(_nan.sum().item())
                        _total = int(_t.numel())
                        _ratio = _nan_count / max(_total, 1)
                        logging.warning(
                            f"[NaN-Scan] batch '{_name}': nan_ratio={_ratio:.6f} all_nan={_nan_count == _total} shape={tuple(_t.shape)}"
                        )
        except Exception:
            pass
