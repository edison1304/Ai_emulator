...
class AtmosphereData:
    ...
    def total_water_path(self) -> torch.Tensor:
        ...
        # Align integration to the actually-present vertical levels in data.
        # This supports cases where data provides a subset of layers (e.g., 9)
        # while the vertical coordinate defines more layers (e.g., 13).
        levels = self.get_present_levels("specific_total_water")
        q = self.specific_total_water
        interface_pressure = self._vertical_coordinate.interface_pressure(
            self.surface_pressure
        )
        device = interface_pressure.device
        idx = torch.as_tensor(levels, device=device, dtype=torch.long)
        # pressure thickness for each present layer L: p[L+1] - p[L]
        thickness = interface_pressure.index_select(-1, idx + 1) - interface_pressure.index_select(
            -1, idx
        )
        return (q * thickness).sum(dim=-1) / GRAVITY

    def get_present_levels(self, standard_name: str) -> list[int]:
        """Return present vertical level indices for a 3D standard name.

        Uses the stacker to discover names in natural order and parses numeric suffixes.
        Returns a sorted list of integer level indices corresponding to the stacked order.
        """
        names = self._stacker.get_all_level_names(standard_name, self.data)
        levels: list[int] = []
        for name in names:
...
def compute_layer_thickness(
    ...
    # The issue is that dlogp and tv have different numbers of levels
    # We need to handle this mismatch appropriately
    if tv.shape[-1] != dlogp.shape[-1]:
        if tv.shape[-1] == dlogp.shape[-1] - 1:
            # Case 1: tv has one less level than dlogp (expected case)
            # Interpolate tv from layer midpoints to layer interfaces
            tv_interp = torch.zeros_like(dlogp)
            tv_interp[..., 0] = tv[..., 0]  # First interface uses first layer
            tv_interp[..., -1] = tv[..., -1]  # Last interface uses last layer
            # Middle interfaces use average of adjacent layers
            tv_interp[..., 1:-1] = 0.5 * (tv[..., :-1] + tv[..., 1:])
            tv = tv_interp
        elif tv.shape[-1] == dlogp.shape[-1] + 1:
            # Case 2: tv has one more level than dlogp (unexpected case)
            # Use the first dlogp.shape[-1] levels of tv to match dlogp
            tv = tv[..., :dlogp.shape[-1]]
        elif tv.shape[-1] == dlogp.shape[-1] - 2:
            # Case 3: tv has two less levels than dlogp (current case: 9 vs 11)
            # This suggests dlogp has extra levels, we need to interpolate tv more
            # For now, use the first 9 levels of dlogp to match tv
            dlogp = dlogp[..., :tv.shape[-1]]
        else:
            raise ValueError(
                f"Tensor shape mismatch: tv has {tv.shape[-1]} levels, "
                f"dlogp has {dlogp.shape[-1]} levels. Expected tv to have "
                f"{dlogp.shape[-1]}, {dlogp.shape[-1] - 1}, or {dlogp.shape[-1] - 2} levels."
            )
