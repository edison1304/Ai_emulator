...
class Stacker:
    ...
    def _natural_sort_names(self, prefix: str, data: TensorMapping) -> list[str]:
        ...
        # Sort by numeric suffix (level). If gaps exist (e.g., missing level indices),
        # proceed in sorted order and emit a warning instead of failing hard.
        sorted_levels = sorted(levels)
        if sorted_levels != list(range(len(sorted_levels))):
            warnings.warn(
                f"Non-contiguous levels for prefix '{prefix}'; "
                f"found levels {sorted_levels}. Proceeding by stacking in sorted order.",
                RuntimeWarning,
            )
        ...
        return [
            ...
            for _, name in sorted(
                zip(levels, names), key=lambda pair: pair[0]
            )
        ]
