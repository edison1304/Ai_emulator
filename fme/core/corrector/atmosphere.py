...
def _force_conserve_dry_air(
    ...
    # Collapse any unexpected non-horizontal dimensions (e.g., a stray time dim)
    # so that both tensors are per-batch scalars with shape [batch, 1, 1].
    def _collapse_non_horizontal_dims(t: torch.Tensor) -> torch.Tensor:
        if t.dim() <= 3:
            return t
        # Reduce all non-horizontal, non-batch dims in one pass
        reduce_dims = tuple(range(1, t.dim() - 2))
        return t.mean(dim=reduce_dims, keepdim=True)
    global_gen_dry_air = _collapse_non_horizontal_dims(global_gen_dry_air)
    global_target_gen_dry_air = _collapse_non_horizontal_dims(global_target_gen_dry_air)
    ...
    # Align ak/bk differences to present moisture levels
    present_levels = gen.get_present_levels("specific_total_water")
    ak_diff_full = vertical_coordinate.get_ak().diff().to(precision)
    bk_diff_full = vertical_coordinate.get_bk().diff().to(precision)
    device = ak_diff_full.device
    idx = torch.as_tensor(present_levels, device=device, dtype=torch.long)
    ak_diff = ak_diff_full.index_select(0, idx)
    bk_diff = bk_diff_full.index_select(0, idx)
