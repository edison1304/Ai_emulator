...
def load_series_data(
    ...
    for n in names:
        ...
        # Ensure time dim present and matches
        if "time" not in variable.dims:
            raise ValueError(f"Variable {n} is missing time dimension")
        # Squeeze singleton non-time dims (e.g., lev=1)
        try:
            drop_dims = [d for d, s in zip(variable.dims, variable.shape) if d != "time" and s == 1]
            if drop_dims:
                var = var.squeeze(drop=True)
                variable = var.variable
        except Exception:
            pass
        # Delegate broadcasting to indexer-based path (handles missing spatial dims)
