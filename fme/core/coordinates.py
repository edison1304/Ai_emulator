...
class HybridSigmaPressureCoordinate(VerticalCoordinate):
    ...
    def vertical_integral(
        ...
        """
        # Check if integrand dimensions are compatible with vertical coordinate
        # Expected: len(ak) = integrand.shape[-1] + 1 (interfaces = layers + 1)
        # But we need to handle cases where this doesn't match exactly
        if len(self.ak) != integrand.shape[-1] + 1:
            # If we have more interfaces than expected, we need to adjust
            if len(self.ak) > integrand.shape[-1] + 1:
                # Case: we have more interfaces than needed
                # Use only the first (integrand.shape[-1] + 1) interfaces
                expected_interfaces = integrand.shape[-1] + 1
                ak_adjusted = self.ak[:expected_interfaces]
                bk_adjusted = self.bk[:expected_interfaces]
            else:
                raise ValueError(
                    f"The last dimension of integrand ({integrand.shape[-1]}) must match "
                    f"the number of vertical layers ({len(self.ak) - 1}) in the hybrid "
                    f"sigma-pressure vertical coordinate."
                )
        else:
            ak_adjusted = self.ak
            bk_adjusted = self.bk
        # Calculate interface pressure using adjusted ak and bk
        interface_pressure = torch.stack(
            [ak + bk * surface_pressure for ak, bk in zip(ak_adjusted, bk_adjusted)],
            dim=-1,
        )
