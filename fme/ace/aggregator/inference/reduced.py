...
class AreaWeightedReducedMetric:
    ...
    def record(self, target: TensorMapping, gen: TensorMapping, i_time_start: int):
        ...
        for name, tensor in new_values.items():
            ...
            if time_slice.stop > self._n_timesteps:
                # Adjust time_slice to fit within the allocated tensor
                adjusted_time_slice = slice(time_slice.start, self._n_timesteps)
                if adjusted_time_slice.start < adjusted_time_slice.stop:
                    # Adjust new_value to match the adjusted slice size
                    adjusted_size = adjusted_time_slice.stop - adjusted_time_slice.start
                    if new_value.shape[0] > adjusted_size:
                        new_value = new_value[:adjusted_size]
                    self._total[name][adjusted_time_slice] += new_value
            else:
